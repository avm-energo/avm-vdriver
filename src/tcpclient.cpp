...
TCPClient::TCPClient(QObject *parent)
    : QObject{parent}, status(false), isinitcom(false) {
...
bool TCPClient::init(const QString &ip, int port) {
  ...
  connect(m_reconnectTimer, &QTimer::timeout, this,
          &TCPClient::itsTimeToReconnect);
  m_ip = ip;
  m_port = port;
  connect(m_socket, &QAbstractSocket::readyRead, this,
          &TCPClient::newDataReceived);
#if QT_VERSION < QT_VERSION_CHECK(5, 15, 0)
  connect(m_socket,
          QOverload<QAbstractSocket::SocketError>::of(&QAbstractSocket::error),
          this, &TCPClient::errorOccured);
#else
...
void TCPClient::errorOccured(QAbstractSocket::SocketError error) {
  ...
  default:
    qCritical() << m_name
                << "The following error occurred: " << m_socket->errorString();
  }
